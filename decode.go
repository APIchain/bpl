...
func getDecoder(t reflect.Type) (DecoderReader, error) {
	...
	default:
		return nil, fmt.Errorf("bpl, type %v, kind %v not support", t, kind)
	}
}
...
func decodeBigInt(v reflect.Value, ctx *DecodeContext) error {
	if ctx.t != EXT16 {
		return fmt.Errorf("bpl: decode type %X, expected %X", ctx.t, EXT16)
	}
	if ctx.ext != EXT_BIGINT {
		return fmt.Errorf("bpl: decode exttype %X, expected %X", ctx.ext, EXT_BIGINT)
	}
	val, err := ctx.readRaw()
	if err != nil {
		return fmt.Errorf("bpl: decode big.Int fail")
	}
...
func decodeBool(v reflect.Value, ctx *DecodeContext) error {
	if ctx.t == TRUE {
		...
		return fmt.Errorf("bpl: decode type %X, expected %X or %X", ctx.t, TRUE, FALSE)
	}
...
func decodeUint8(v reflect.Value, ctx *DecodeContext) error {
	if ctx.t != UINT8 {
		return fmt.Errorf("bpl: decode type %X, expected %X", ctx.t, UINT8)
	}
	val, err := ctx.readUint()
	if err != nil {
		return fmt.Errorf("bpl: decode uint8 fail")
	}
	v.SetUint(val)
	return nil
}
...
func decodeUint16(v reflect.Value, ctx *DecodeContext) error {
	if ctx.t != UINT16 {
		return fmt.Errorf("bpl: decode type %X, expected %X", ctx.t, UINT16)
	}
	val, err := ctx.readUint()
	if err != nil {
		return fmt.Errorf("bpl: decode uint16 fail")
	}
	v.SetUint(val)
	return nil
}
...
func decodeUint32(v reflect.Value, ctx *DecodeContext) error {
	if ctx.t != UINT32 {
		return fmt.Errorf("bpl: decode type %X, expected %X", ctx.t, UINT32)
	}
	val, err := ctx.readUint()
	if err != nil {
		return fmt.Errorf("bpl: decode uint32 fail")
	}
	v.SetUint(val)
	return nil
}
...
func decodeUint64(v reflect.Value, ctx *DecodeContext) error {
	if ctx.t != UINT64 {
		return fmt.Errorf("bpl: decode type %X, expected %X", ctx.t, UINT64)
	}
	val, err := ctx.readUint()
	if err != nil {
		return fmt.Errorf("bpl: decode uint64 fail")
	}
	v.SetUint(val)
	return nil
}
...
func decodeString(v reflect.Value, ctx *DecodeContext) error {
	if ctx.t != STR16 {
		return fmt.Errorf("bpl: decode type %X, expected %X", ctx.t, STR16)
	}
	val, err := ctx.readRaw()
	if err != nil {
		return fmt.Errorf("bpl: decode string fail")
	}
...
func decodeBytes(v reflect.Value, ctx *DecodeContext) error {
	if ctx.t != BIN16 {
		return fmt.Errorf("bpl: decode type %X, expected %X", ctx.t, BIN16)
	}
	val, err := ctx.readRaw()
	if err != nil {
		return fmt.Errorf("bpl: decode bin16 fail")
	}
	v.SetBytes(val)
	return nil
}
...
func decodeByteArray(v reflect.Value, ctx *DecodeContext) error {
	...
	vlen := v.Len()
	if len(val) != vlen {
		return fmt.Errorf("bpl: wrong array size")
	}
...
func decodeArray(v reflect.Value, ctx *DecodeContext, elemDecoder DecoderReader) error {
	...
	if vlen != int(ctx.size) {
		return fmt.Errorf("bpl: wrong array size %v, expected %v", int(ctx.size), vlen)
	}
	i := 0
	for ; i < vlen; i++ {
	...
	if i < vlen {
		return fmt.Errorf("bpl decoder: array size array")
	}
	return nil
}
...
func decodeSlice(v reflect.Value, ctx *DecodeContext, elemDecoder DecoderReader) error {
	...
	for ; i < asize; i++ {
		...
		if err := elemDecoder(v.Index(i), ctx); err != nil {
			return fmt.Errorf("bpl decoder: slice decode error")
		}
	}
	return nil
}
